...
function parseExcelCEBA(data, fileName) {
    ...
    for (const sname of wb.SheetNames) {
        ...
        if (nombre || items.length > 0) {
            results.push({
                id: `CETPRO-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
...
function parsePronoepsaStyle(data, fileName) {
    const wb = XLSX.read(data, { type: "array" });
    if (!wb.SheetNames.includes("Datos CEBA") && !wb.SheetNames.includes("Datos CETPRO")) return [];

    const sheetName = wb.SheetNames.includes("Datos CETPRO") ? "Datos CETPRO" : "Datos CEBA";
    const wsDatos = wb.Sheets[sheetName];
    ...
    const items = [];
    for (const sname of wb.SheetNames) {
        if (sname === "Datos CEBA" || sname === "Datos CETPRO") continue;
        ...
        reader.onload = (ev) => {
            try {
                ...
                if (wb.SheetNames.includes("Datos CEBA")) {
                    parsed = parsePronoepsaStyle(data, file.name);  /* CETPRO */
                } else {
    ...
    return (
        <div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 14 }}>
                <div>
                    <h2 style={{ color: C.navy1, fontSize: "1.35rem", margin: 0, fontFamily: "'DM Serif Display',serif" }}>Requerimientos CETPRO</h2>
                    <p style={{ color: C.g500, fontSize: "0.82rem", margin: "4px 0 0", fontFamily: "'DM Sans'" }}>Analisis de requerimientos anuales — {cebas.length} CETPRO registrados — {new Date().getFullYear()}</p>
                </div>
                <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                    <div style={{ display: "flex", borderRadius: 8, overflow: "hidden", border: `1px solid ${C.g200}` }}>
                        {[["dashboard", "Dashboard"], ["cebas", "CETPRO"], ["items", "Items"], ["carga", "Carga"]].map(([v, l]) => <button key={v} onClick={() => setView(v)} style={btnTab(view === v)}>{l}</button>)}
                    </div>
                </div>
            </div>
            ...
            {view === "dashboard" && cebas.length > 0 && (
                <div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(155px, 1fr))", gap: 12, marginBottom: 20 }}>
                        {[
                            { label: "CETPRO Registrados", value: kpis.totalCebas, border: C.navy4, icon: Icons.grid },
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16, marginBottom: 16 }} className="grid-calendar">
                        <div style={{ ...card, padding: 20 }}>
                            <h3 style={h3s}>Comparativo por CETPRO — Estudiantes vs Requerimientos</h3>
                    ...
                    {showViabilidad && impactoData.length > 0 && (
                        <div style={{ ...card, padding: 20, marginBottom: 16, borderTop: `3px solid ${C.purple}` }}>
                            <h3 style={{ ...h3s, display: "flex", alignItems: "center", gap: 8 }}>{Icons.shield(16, C.purple)} Indice de Impacto Presupuestal por CETPRO</h3>
                ...
                <div>
                    {filteredCebas.length === 0 ? (
                        <div style={{ ...card, textAlign: "center", padding: 60 }}>
                            ...
                            <h3 style={{ color: C.navy1, fontSize: "1.15rem", margin: "16px 0 8px", fontFamily: "'DM Serif Display',serif" }}>Sin CETPRO registrados</h3>
            ...
            {view === "carga" && (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }} className="grid-calendar">
                    ...
                    <div style={{ ...card, padding: 24 }}>
                        <h3 style={h3s}>Carga de Requerimientos CETPRO</h3>
                        <p style={{ color: C.g500, fontSize: "0.78rem", margin: "-10px 0 16px", fontFamily: "'DM Sans'" }}>Suba los archivos Excel (.xlsx) enviados por los CETPRO con sus fichas de requerimientos.</p>
                        <div
                        ...
                        >
                            ...
                            {processing ? (
                                ...
                                <>
                                    ...
                                    <div style={{ color: C.navy1, fontSize: 13, fontWeight: 600, fontFamily: "'DM Sans'", marginBottom: 4 }}>Arrastre archivos Excel de CETPRO aqui</div>
                        ...
                        {cebas.length > 0 && (
                            <div style={{ marginTop: 20 }}>
                                <div style={{ fontSize: "0.72rem", fontWeight: 700, color: C.g500, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8, fontFamily: "'DM Sans'" }}>CETPRO Cargados ({cebas.length})</div>
                                {cebas.map(c => (
            ...
            {view === "dashboard" && cebas.length === 0 && (
                <div style={{ ...card, textAlign: "center", padding: 60 }}>
                    ...
                    <p style={{ color: C.g500, fontSize: "0.85rem", fontFamily: "'DM Sans'", maxWidth: 420, margin: "0 auto" }}>
                        Suba los archivos Excel de requerimientos de los CETPRO desde la vista "Carga" para ver el dashboard de analisis.
                    </p>
